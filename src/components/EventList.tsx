import React from "react";
import { useAppSelector } from "../store/store";
import type { EventListProps } from "../utils/interfaces";
import { formatTimeTo12Hour } from "../utils/utilities";
import Loader from "./Loader";

const EventList: React.FC<EventListProps> = React.memo(
  ({
    selectedDate = new Date(),
    events,
    emptyMessage,
    scrollable = false,
    className,
  }) => {
    const { isLoading } = useAppSelector((state) => state.sessionSlices);

    const currentDate = new Date(selectedDate);
    const dateAndMonth = currentDate.toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
    });
    return (
      <div>
        {isLoading ? (
          <Loader isBlue={true} padding={10} />
        ) : events?.length > 0 ? (
          <div
            className={`${
              scrollable
                ? "overflow-auto max-h-[450px] lg:max-h-[325px]"
                : "mt-5"
            }`}
          >
            {events.map(
              ({ name, dogImageUrl, description, startTime, endTime }, idx) => (
                <div
                  key={idx}
                  className={`flex items-center rounded-xl ${className} ${
                    idx !== 0 &&
                    (scrollable ? "xxs:mt-2 lg:mt-3" : "xxs:mt-3 lg:mt-5")
                  }`}
                >
                  {dogImageUrl && (
                    <img
                      src={dogImageUrl}
                      className="w-[65px] h-[65px] rounded-xl object-cover"
                    />
                  )}
                  <div className="space-y-1">
                    <div
                      className={`${
                        scrollable
                          ? "text-sm font-bold"
                          : "text-base font-semibold"
                      }`}
                    >
                      {name}
                    </div>
                    <div className="text-xs">{description}</div>
                    <div className="text-xs">
                      {dateAndMonth} | {formatTimeTo12Hour(startTime)} -{" "}
                      {formatTimeTo12Hour(endTime)}
                    </div>
                  </div>
                </div>
              )
            )}
          </div>
        ) : (
          <h3 className="xxs:text-sm sm:text-base text-red-400 py-5 font-semibold text-center">
            {emptyMessage}
          </h3>
        )}
      </div>
    );
  }
);

export default EventList;
